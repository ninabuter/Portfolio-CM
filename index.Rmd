...
workflow() %>% 
  ...
  coord_flip() +
  theme_minimal() +
  labs(title = "Prediction Model For Beyoncé's Musical Features", x = NULL, y = "Importance")
```

***
In this graph, you can see that **timbre coefficient c02** stands out as the most important feature. This is remarkable, because c02 has shown to be the most different timbre coefficient between the albums, as you could have seen on the preceding page about timbre coefficients. Maybe this also indicates why this feature is so important in Beyoncé's music. 

### K-Nearest Neighbour with **timbre coefficient c02**, algorithm can make a fair prediction on Beyoncé's older and newer music based on this coefficient. 
...
***
Here, Beyoncé's albums and the accompanying songs are divided in two groups, namely **old** (her first four albums, untill *Beyoncé*) and **new** (her last three albums). The algorithm has made predictions about the distribution of the songs in the two groups based on the feature c02 (since this feature stood out in the graphs of the preceding pages).
...
In this corpus, I've analysed four different things, namely: general features of the albums, pitch and timbre features of the song *Love On Top*, tempo, and timbre features. It was very interesting to see the possibilities of Spotify API together with RStudio. So I've not only learned a lot about Beyoncé's music during this course, but also about programming and visualization. 

The main things I've learned about Beyoncé's music is that:

* Valence and acousticness have decreased over time;
...
* The album *Beyoncé* shows a lot of similarity in tempo;
* Timbre feature c02 stands out in Beyoncé music and has decreased in value over time, which means lower frequencies like bass have become more prominent and that this can define Beyoncé's music to a high degree. 

It's impressing how much I've learned about Beyoncé's music only in a few weeks time by Spotify API, and therefore I'm very excited and curious what the future will bring, because I'm sure I'll use this again in the future for very interesting new researches. 